...
goog.needsTranspile_ = function(a) {
    ...
    if (!goog.transpiledLanguages_) {
        ...
        try {
            ...
            eval('(()=>{"use strict";class X{constructor(){if(new.target!=String)throw 1;this.x=42}}let q=Reflect.construct(X,[],String);if(q.x!=42||!(q instanceof String))throw 1;for(const a of[2,3]){if(a==2)continue;function f(z={a}){let a=0;return z.a}{function f(){return 0;}}return f()==3}})()') && (goog.transpiledLanguages_.es6 = !1)
        } catch (b) {}
    }
...
Blockly.onMouseMove_ = function(a) {
    ...
    if (b.dragMode_ != Blockly.DRAG_NONE) {
        ...
        b.scroll(b.startScrollX + c, b.startScrollY + d);
...
Blockly.Workspace.prototype.renameVariable = function(a, b) {
    ...
    c == d || -1 != c && -1 == d ? this.variableList[c] = b : -1 != c && -1 != d ? (this.variableList.splice(c, 1),
    this.variableList[d] = b) : (this.variableList.push(b),
...
Blockly.Workspace.prototype.fireChangeListener = function(a) {
    ...
    for (var b = this.listeners_.slice(), c = 0, d; d = b[c]; c++)
        d(a)
}
;
Blockly.Workspace.prototype.getBlockById = function(a) {
    var b = this.blockDB_[a];
    !b && this.getFlyout() && this.getFlyout().getWorkspace() && (b = this.getFlyout().getWorkspace().blockDB_[a]);
    return b || null
}
;
...
Blockly.Bubble = function(a, b, c, d, e, f) {
    ...
    this.renderArrow_();
    this.rendered_ = !0;
    a.options.readOnly || (Blockly.bindEvent_(this.bubbleBack_, "mousedown", this, this.bubbleMouseDown_),
...
Blockly.Bubble.prototype.bubbleMouseDown_ = function(a) {
    ...
    Blockly.Bubble.onMouseUpWrapper_ = Blockly.bindEvent_(document, "mouseup", this, Blockly.Bubble.bubbleMouseUp_),
...
Blockly.Comment.prototype.createEditor_ = function() {
    ...
    Blockly.bindEvent_(b, "change", this, function() {
...
Blockly.Connection.prototype.connect_ = function(a) {
    ...
    if (b.isConnected()) {
        ...
        if (g.isShadow())
            ...
            for (var k = d; k.nextConnection; ) {
                ...
                if (l && !l.isShadow())
                    ...
                    g.previousConnection.checkType_(k.nextConnection) && (k.nextConnection.connect(g.previousConnection),
                    g = null);
                    break
                }
            }
        ...
        Blockly.Events.recordUndo)) {
            ...
            setTimeout(function() {
                g.workspace && !g.getParent() && (Blockly.Events.setGroup(m),
                g.outputConnection ? g.outputConnection.bumpAwayFrom_(b) : g.previousConnection && g.previousConnection.bumpAwayFrom_(b),
                Blockly.Events.setGroup(!1))
            }, Blockly.BUMP_DELAY)
        }
        b.setShadowDom(h)
...
Blockly.Connection.prototype.isConnectionAllowed = function(a) {
    if (a.sourceBlock_.isInsertionMarker() || this.canConnectWithReason_(a) != Blockly.Connection.CAN_CONNECT)
        return !1;
    ...
    case Blockly.PREVIOUS_STATEMENT:
        if (!b || this != b) {
            ...
            if (a.targetConnection)
                return a.targetConnection == Blockly.insertionMarkerConnection_ ? !0 : !1
        }
        if (b)
            if (this == b) {
                if (this.targetConnection)
                    return !1
            } else if (this == this.sourceBlock_.nextConnection && a.isConnectedToNonInsertionMarker())
                return !1;
        break;
    case Blockly.OUTPUT_VALUE:
        return !1;
    case Blockly.INPUT_VALUE:
        if (a.targetConnection && !a.targetBlock().isMovable() && !a.targetBlock().isShadow())
            return !1;
        break;
    case Blockly.NEXT_STATEMENT:
        if (b && this == this.sourceBlock_.previousConnection && a.isConnectedToNonInsertionMarker() && !b.targetConnection || a.isConnectedToNonInsertionMarker() && !this.sourceBlock_.nextConnection && !a.targetBlock().isShadow() && a.targetBlock().nextConnection)
            return !1;
        break;
    default:
...
Blockly.Field.prototype.init = function() {
    if (!this.fieldGroup_) {
        ...
        this.mouseUpWrapper_ = Blockly.bindEvent_(this.getClickTarget_(), "mouseup", this, this.onMouseUp_);
...
Blockly.Field.prototype.render_ = function() {
    if (this.visible_ && this.textElement_) {
        ...
        this.EDITABLE && (b += Blockly.BlockSvg.EDITABLE_FIELD_PADDING);
        var c = 0;
        this.positionArrow && (c = this.positionArrow(b),
        b += c);
        this.box_ && (b += 2 * Blockly.BlockSvg.BOX_FIELD_PADDING);
        a = (b - c) / 2;
        this.sourceBlock_.RTL && (a += c);
        this.sourceBlock_.isShadow() && !this.positionArrow && (c = Blockly.BlockSvg.FIELD_WIDTH / 2,
        a = this.sourceBlock_.RTL ? Math.min(b - c, a) : Math.max(c, a));
        this.textElement_.setAttribute("x", a)
    } else
        b = 0;
    this.size_.width = b;
    this.box_ && (this.box_.setAttribute("width", this.size_.width),
...
Blockly.Field.prototype.updateTextNode_ = function() {
    if (this.textElement_) {
        ...
        this.textElement_.setAttribute("class", "blocklyText blocklyTextTruncated")) : this.textElement_.setAttribute("class", "blocklyText");
...
Blockly.Field.prototype.getClickTarget_ = function() {
    for (var a = 0, b = 0, c; c = this.sourceBlock_.inputList[b]; b++)
        a += c.fieldRow.length;
...
Blockly.DropDownDiv.createDom = function() {
    Blockly.DropDownDiv.DIV_ || (Blockly.DropDownDiv.DIV_ = goog.dom.createDom("div", "blocklyDropDownDiv"),
...
Blockly.DropDownDiv.showPositionedByBlock = function(a, b, c, d) {
    ...
    f *= e;
    a *= e;
    e = goog.style.getPageOffset(b.getSvgRoot());
    f = e.x + f / 2;
    a = e.y + a;
    e = e.y;
    d && (e += d);
...
Blockly.DropDownDiv.getPositionMetrics = function(a, b, c, d) {
    var e = Blockly.DropDownDiv.DIV_, f = goog.style.getPageOffset(Blockly.DropDownDiv.boundsElement_), g = goog.style.getSize(Blockly.DropDownDiv.boundsElement_), e = goog.style.getSize(e), h;
    b + e.height > f.y + g.height ? d - e.height < f.y ? (c = b + Blockly.DropDownDiv.PADDING_Y,
    h = !1) : (a = c,
    c = d - e.height - Blockly.DropDownDiv.PADDING_Y,
    h = !0) : (c = b + Blockly.DropDownDiv.PADDING_Y,
    h = !1);
    var k = a - Blockly.DropDownDiv.ARROW_SIZE / 2
      , k = Math.max(f.x, Math.min(k, f.x + g.width));
    a = Math.max(f.x, Math.min(a - e.width / 2, f.x + g.width - e.width));
...
Blockly.DropDownDiv.hideIfOwner = function(a) {
    return Blockly.DropDownDiv.owner_ === a ? (Blockly.DropDownDiv.hide(),
    !0) : !1
}
;
...
Blockly.Options = function(a) {
    ...
    if (b)
        ...
        a.toolbox || (c = (new DOMParser).parseFromString(Blockly.Blocks.defaultToolbox, "text/xml"),
        a.toolbox = c.children[0]),
    ...
    var n = a.toolboxPosition
      , n = "end" === n ? !1 : !0
      , n = m ? n ? Blockly.TOOLBOX_AT_TOP : Blockly.TOOLBOX_AT_BOTTOM : n == l ? Blockly.TOOLBOX_AT_RIGHT : Blockly.TOOLBOX_AT_LEFT
      , p = a.scrollbars;
    void 0 === p && (p = d);
    var t = a.css;
    void 0 === t && (t = !0);
    ...
    var v = !!a.realtime
      , w = v ? a.realtimeOptions : void 0
      , q = a.colours;
    if (q)
        for (var r in q)
            q.hasOwnProperty(r) && Blockly.Colours.hasOwnProperty(r) && (Blockly.Colours[r] = q[r]);
    this.RTL = l;
    this.collapse = f;
    this.comments = g;
    this.disable = h;
    ...
    this.enableRealtime = v;
    this.realtimeOptions = w;
    this.toolboxPosition = n
}
;
...
Blockly.Trashcan.prototype.setOpen_ = function(a) {
    this.isOpen != a && (clearTimeout(this.lidTask_),
    this.isOpen = a,
    this.animateLid_())
}
;
...
Blockly.WorkspaceSvg.prototype.addFlyout_ = function() {
    ...
    this.flyout_ = this.horizontalLayout ? new Blockly.HorizontalFlyout(a) : new Blockly.VerticalFlyout(a);
    this.flyout_.autoClose = !1;
    a = this.flyout_.createDom();
...
Blockly.WorkspaceSvg.prototype.resizeContents = function() {
    this.resizesEnabled_ && (this.scrollbar && this.scrollbar.resize(),
    this.updateInverseScreenCTM())
}
;
...
Blockly.WorkspaceSvg.prototype.getFlyout = function() {
    return this.flyout_ ? this.flyout_ : this.toolbox_ && this.toolbox_.flyout_ ? this.toolbox_.flyout_ : null
}
;
Blockly.WorkspaceSvg.prototype.translate = function(a, b) {
    ...
    this.dragSurface && this.dragSurface.translateAndScaleGroup(a, b, this.scale)
}
;
...
Blockly.WorkspaceSvg.prototype.reportValue = function(a, b) {
    ...
    e.innerHTML = Blockly.encodeEntities(b);
    d.appendChild(e);
        ...
        try {
            ...
            if (!isNaN(c) && !isNaN(d)) {
                ...
                do {
                    ...
                    if (!a)
                        for (var k = b.getConnections_(!1), f = 0, l; l = k[f]; f++)
...
Blockly.WorkspaceSvg.prototype.onMouseDown_ = function(a) {
    ...
    setTimeout("if(!$('#blocklyDiv').hasClass('simple')){$('.blocklyFlyout').hide();$('.scratchCategoryMenuItem ').removeClass('categorySelected')}",'100'),   //add by rongw 掩藏flyout 
    
    "mouseup"in Blockly.Touch.TOUCH_MAP && (Blockly.Touch.onTouchUpWrapper_ = Blockly.Touch.onTouchUpWrapper_ || [],
    Blockly.Touch.onTouchUpWrapper_ = Blockly.Touch.onTouchUpWrapper_.concat(Blockly.bindEvent_(document, "mouseup", null, Blockly.onMouseUp_))),
...
Blockly.WorkspaceSvg.prototype.showContextMenu_ = function(a) {
    ...
    if (!this.options.readOnly && !this.isFlyout) {
        ...
        for (var n = [], k = 0; k < e.length; k++)
            b(e[k]);
        for (var p = 0, k = 0; k < n.length; k++)
            n[k].isShadow() || p++;
        g = {
            text: 1 == p ? Blockly.Msg.DELETE_BLOCK : Blockly.Msg.DELETE_X_BLOCKS.replace("%1", String(p)),
            enabled: 0 < p,
            callback: function() {
                (2 > p || window.confirm(Blockly.Msg.DELETE_ALL_BLOCKS.replace("%1", String(p)))) && c()
            }
        };
        d.push(g);
...
Blockly.WorkspaceSvg.prototype.playAudio = function(a, b) {
    var c = new Blockly.Events.Ui(null,"sound",null,a);
    c.workspaceId = this.id;
    Blockly.Events.fire(c);
    if (c = this.SOUNDS_[a]) {
        var d = new Date;
...
Blockly.Block = function(a, b, c) {
    ...
    this.id = !c || a.getBlockById(c) || d && d.getBlockById(c) ? Blockly.genUid() : c;
...
Blockly.Block.prototype.getDescendants = function(a) {
    for (var b = [this], c, d = 0; c = this.childBlocks_[d]; d++)
        a && c.isShadow_ || b.push.apply(b, c.getDescendants(a));
    return b
}
;
...
Blockly.Block.prototype.setInsertionMarker = function(a) {
    this.isInsertionMarker_ != a && (this.isInsertionMarker_ = a) && (this.setColour(Blockly.Colours.insertionMarker),
...
Blockly.Block.prototype.setColour = function(a, b, c) {
    ...
    this.colourSecondary_ = void 0 !== b ? this.makeColour_(b) : goog.color.darken(goog.color.hexToRgb(this.colour_), .1);
    this.colourTertiary_ = void 0 !== c ? this.makeColour_(c) : goog.color.darken(goog.color.hexToRgb(this.colour_), .2);
...
Blockly.Block.prototype.jsonInit = function(a) {
    ...
    void 0 !== a.colour && this.setColour(a.colour, a.colourSecondary, a.colourTertiary);
...
Blockly.Block.prototype.interpolate_ = function(a, b, c) {
    ...
    !a.length || "string" != typeof a[a.length - 1] && 0 != a[a.length - 1].type.indexOf("field_") || (g = {
        type: "input_dummy"
    },
    c && (g.align = c),
    a.push(g));
            ...
            do
                ...
                "string" == typeof e)
                    ...
                    case "field_textdropdown":
                        f = new Blockly.FieldTextDropdown(e.text,e.options);
                        "boolean" == typeof e.spellcheck && f.setSpellcheck(e.spellcheck);
                        break;
                    case "field_numberdropdown":
                        f = new Blockly.FieldNumberDropdown(e.value,e.options,e.min,e.max,e.precision);
                        break;
                    case "field_angle":
                    ...
                    case "field_dropdown":
                        ...
                        break;
                    case "field_iconmenu":
                        f = new Blockly.FieldIconMenu(e.options);
                        break;
                    case "field_image":
                        f = new Blockly.FieldImage(e.src,e.width,e.height,e.alt,e.flip_rtl);
                        break;
                    case "field_number":
...
Blockly.RenderedConnection.prototype.highlight = function() {
    var a;
    a = "m -20,0 h 5 " + Blockly.BlockSvg.NOTCH_PATH_LEFT + " h 5";
...
Blockly.BlockSvg.prototype.initInputShape = function(a) {
    this.inputShapes_[a.name] || (this.inputShapes_[a.name] = Blockly.createSvgElement("path", {
        "class": "blocklyPath",
...
Blockly.BlockSvg.terminateDrag = function() {
    ...
    if (Blockly.dragMode_ == Blockly.DRAG_FREE && a) {
        Blockly.replacementMarker_ ? Blockly.BlockSvg.removeReplacementMarker() : Blockly.insertionMarker_ && (Blockly.Events.disable(),
        Blockly.insertionMarkerConnection_ && Blockly.BlockSvg.disconnectInsertionMarker(),
...
Blockly.BlockSvg.prototype.setParent = function(a) {
    if (a != this.parentBlock_) {
        ...
        if (this.parentBlock_ && b) {
            ...
            Blockly.selected != this && (this.workspace.getCanvas().appendChild(b),
        ...
        this.moveConnections_(b.x - c.x, b.y - c.y),
        this.isShadow() && this.setColour(this.getColour(), this.getColourSecondary(), a.getColourTertiary()))
    }
}
;
Blockly.BlockSvg.prototype.getRelativeToSurfaceXY = function() {
    var a = 0
      , b = 0
      , c = this.workspace.dragSurface ? this.workspace.dragSurface.getGroup() : null
      , d = this.getSvgRoot();
    if (d) {
        do {
            var e = Blockly.getRelativeXY_(d)
              , a = a + e.x
              , b = b + e.y;
            this.workspace.dragSurface && this.workspace.dragSurface.getCurrentBlock() == d && (e = this.workspace.dragSurface.getSurfaceTranslation(),
            a += e.x,
            b += e.y);
            d = d.parentNode
...
Blockly.BlockSvg.prototype.moveBy = function(a, b) {
    ...
    if (c)
        var d = new Blockly.Events.Move(this);
...
;
Blockly.BlockSvg.prototype.translate = function(a, b, c) {
...
Blockly.BlockSvg.prototype.getBoundingRectangle = function() {
    var a = this.getRelativeToSurfaceXY(this), b = this.getHeightWidth(), c;
    this.RTL ? (c = new goog.math.Coordinate(a.x - b.width,a.y),
    a = new goog.math.Coordinate(a.x,a.y + b.height)) : (c = new goog.math.Coordinate(a.x,a.y),
...
Blockly.BlockSvg.prototype.onMouseUp_ = function(a) {
    var b = Blockly.WidgetDiv.isVisible() || Blockly.DropDownDiv.isVisible();
    Blockly.Touch.clearTouchIdentifier();
    Blockly.dragMode_ == Blockly.DRAG_FREE || b || (Blockly.Events.fire(new Blockly.Events.Ui(this,"click",void 0,void 0)),
    b = this.workspace.getBlockById(this.id).getRootBlock(),
    Blockly.Events.fire(new Blockly.Events.Ui(b,"stackclick",void 0,void 0)));
    Blockly.terminateDrag_();
    !this.getParent() && Blockly.selected.isDeletable() && this.workspace.isDeleteArea(a) ? ((a = this.workspace.trashcan) && setTimeout(a.close.bind(a), 100),
    Blockly.selected.dispose(!1, !0)) : Blockly.selected && Blockly.highlightedConnection_ && (this.positionNewBlock(Blockly.selected, Blockly.localConnection_, Blockly.highlightedConnection_),
    ...
    Blockly.highlightedConnection_ && (Blockly.highlightedConnection_ = null);
    Blockly.Css.setCursor(Blockly.Css.Cursor.OPEN);
...
Blockly.BlockSvg.prototype.setDragging_ = function(a) {
    a ? (this.addDragging(),
    Blockly.draggingConnections_ = Blockly.draggingConnections_.concat(this.getConnections_(!0))) : (this.removeDragging(),
...
Blockly.BlockSvg.prototype.moveToDragSurface_ = function() {
    ...
    this.workspace.dragSurface.translateSurface(a.x, a.y);
    console.log("1111");
...
Blockly.BlockSvg.prototype.moveOffDragSurface_ = function() {
    var a = this.getRelativeToSurfaceXY();
    this.clearTransformAttributes_();
    this.translate(a.x, a.y, !1);
    this.workspace.dragSurface.clearAndHide(this.workspace.getCanvas())
}
;
Blockly.BlockSvg.prototype.clearTransformAttributes_ = function() {
    this.getSvgRoot().hasAttribute("transform") && this.getSvgRoot().removeAttribute("transform");
    this.getSvgRoot().hasAttribute("style") && this.getSvgRoot().removeAttribute("style")
}
;
Blockly.BlockSvg.prototype.onMouseMove_ = function(a) {
    if ("mousemove" == a.type && 1 >= a.clientX && 0 == a.clientY && 0 == a.button)
        ...
        Blockly.dragMode_ == Blockly.DRAG_STICKY && goog.math.Coordinate.distance(b, c) * this.workspace.scale > Blockly.DRAG_RADIUS && (Blockly.Css.setCursor(Blockly.Css.Cursor.CLOSED),
        ...
        Blockly.WidgetDiv.hide(!0),
        Blockly.DropDownDiv.hideWithoutAnimation(),
        this.parentBlock_ && this.unplug(),
        this.setDragging_(!0));
        Blockly.dragMode_ == Blockly.DRAG_FREE && this.handleDragFree_(b, c, a);
        a.stopPropagation();
        a.preventDefault()
    }
}
;
Blockly.BlockSvg.prototype.handleDragFree_ = function(a, b, c) {
    var d = goog.math.Coordinate.difference(a, this.dragStartXY_);
    this.workspace.dragSurface.translateSurface(b.x, b.y);
    console.log("23");
    for (var e = 0; e < this.draggedBubbles_.length; e++)
        a = this.draggedBubbles_[e],
        a.bubble.setIconLocation(goog.math.Coordinate.sum(a, d));
    var f = this.getConnections_(!1);
    (a = this.lastConnectionInStack()) && a != this.nextConnection && f.push(a);
    var g = null
      , h = null
      , k = Blockly.SNAP_RADIUS;
    Blockly.localConnection_ && Blockly.highlightedConnection_ && (k = Blockly.CONNECTING_SNAP_RADIUS);
    for (e = 0; e < f.length; e++) {
        var l = f[e]
          , m = l.closest(k, d);
        m.connection && (g = m.connection,
        h = l,
        k = m.radius)
    }
    e = !0;
    h && h.type == Blockly.OUTPUT_VALUE ? e = !0 : Blockly.localConnection_ && Blockly.highlightedConnection_ && (f = Blockly.localConnection_.x_ + d.x - Blockly.highlightedConnection_.x_,
    d = Blockly.localConnection_.y_ + d.y - Blockly.highlightedConnection_.y_,
    d = Math.sqrt(f * f + d * d),
    g && k > d - Blockly.CURRENT_CONNECTION_PREFERENCE && (e = !1));
    e && this.updatePreviews(g, h, k, c, b.x - this.dragStartXY_.x, b.y - this.dragStartXY_.y, h == a)
}
;
Blockly.BlockSvg.prototype.updatePreviews = function(a, b, c, d, e, f, g) {
    Blockly.Events.disable();
    Blockly.highlightedConnection_ && Blockly.highlightedConnection_ != a && (Blockly.replacementMarker_ ? Blockly.BlockSvg.removeReplacementMarker() : Blockly.insertionMarker_ && Blockly.insertionMarkerConnection_ && Blockly.BlockSvg.disconnectInsertionMarker(),
    Blockly.insertionMarker_ && (g && Blockly.localConnection_.sourceBlock_ == this || !g && Blockly.localConnection_.sourceBlock_ != this) && (Blockly.insertionMarker_.dispose(),
    Blockly.insertionMarker_ = null),
    Blockly.highlightedConnection_ = null,
    Blockly.localConnection_ = null);
    a && a != Blockly.highlightedConnection_ && !a.sourceBlock_.isInsertionMarker() && (Blockly.highlightedConnection_ = a,
    Blockly.localConnection_ = b,
    b.type == Blockly.OUTPUT_VALUE || b.type == Blockly.PREVIOUS_STATEMENT && a.isConnected() && !this.nextConnection ? this.addReplacementMarker_(b, a) : this.connectInsertionMarker_(b, a));
    Blockly.Events.enable();
    this.isDeletable() && this.workspace.isDeleteArea(d)
}
;
Blockly.BlockSvg.prototype.addReplacementMarker_ = function(a, b) {
    b.targetBlock() ? (Blockly.replacementMarker_ = b.targetBlock(),
    Blockly.replacementMarker_.highlightForReplacement(!0)) : a.type == Blockly.OUTPUT_VALUE && (Blockly.replacementMarker_ = b.sourceBlock_,
    Blockly.replacementMarker_.highlightShapeForInput(b, !0))
}
;
Blockly.BlockSvg.removeReplacementMarker = function() {
    Blockly.highlightedConnection_.type != Blockly.INPUT_VALUE || Blockly.highlightedConnection_.isConnected() ? Blockly.replacementMarker_.highlightForReplacement(!1) : Blockly.replacementMarker_.highlightShapeForInput(Blockly.highlightedConnection_, !1);
    Blockly.replacementMarker_ = null
}
;
Blockly.BlockSvg.prototype.connectInsertionMarker_ = function(a, b) {
    var c = Blockly.localConnection_.sourceBlock_;
    if (!Blockly.insertionMarker_) {
        Blockly.insertionMarker_ = this.workspace.newBlock(c.type);
        if (c.mutationToDom) {
            var d = c.mutationToDom();
            Blockly.insertionMarker_.domToMutation(d)
        }
        Blockly.insertionMarker_.setInsertionMarker(!0, c.width);
        Blockly.insertionMarker_.initSvg()
    }
    c = Blockly.insertionMarker_;
    d = c.getMatchingConnection(a.sourceBlock_, a);
    d != Blockly.insertionMarkerConnection_ && (c.rendered = !0,
    c.render(),
    c.getSvgRoot().setAttribute("visibility", "visible"),
    this.positionNewBlock(c, d, b),
    d.type != Blockly.PREVIOUS_STATEMENT || c.nextConnection || (Blockly.bumpedConnection_ = b.targetConnection),
    d.connect(b),
    Blockly.insertionMarkerConnection_ = d)
}
;
Blockly.BlockSvg.disconnectInsertionMarker = function() {
    if (Blockly.insertionMarkerConnection_ != Blockly.insertionMarker_.nextConnection || Blockly.insertionMarker_.previousConnection && Blockly.insertionMarker_.previousConnection.targetConnection)
        if (Blockly.insertionMarkerConnection_.type == Blockly.NEXT_STATEMENT && Blockly.insertionMarkerConnection_ != Blockly.insertionMarker_.nextConnection) {
            var a = Blockly.insertionMarkerConnection_.targetConnection;
            a.sourceBlock_.unplug(!1);
            var b = Blockly.insertionMarker_.previousConnection ? Blockly.insertionMarker_.previousConnection.targetConnection : null;
            Blockly.insertionMarker_.unplug(!0);
            b && b.connect(a)
        } else
            Blockly.insertionMarker_.unplug(!0);
    else
        Blockly.insertionMarkerConnection_.targetBlock().unplug(!1);
...
Blockly.BlockSvg.FIELD_TOP_PADDING = .25 * Blockly.BlockSvg.GRID_UNIT;
...
Blockly.BlockSvg.MIN_BLOCK_X = 8 * Blockly.BlockSvg.GRID_UNIT;
...
Blockly.BlockSvg.NOTCH_PATH_DOWN = "c 0,2 1,3 2,4 l 4,4 c 1,1 2,2 2,4 v 12 c 0,2 -1,3 -2,4 l -4,4 c -1,1 -2,2 -2,4";
Blockly.BlockSvg.NOTCH_PATH_UP = "c 0,-2 1,-3 2,-4 l 4,-4 c 1,-1 2,-2 2,-4 v -12 c 0,-2 -1,-3 -2,-4 l -4,-4 c -1,-1 -2,-2 -2,-4";
...
Blockly.BlockSvg.TOP_LEFT_CORNER = "A " + Blockly.BlockSvg.CORNER_RADIUS + "," + Blockly.BlockSvg.CORNER_RADIUS + " 0 0,0 0," + Blockly.BlockSvg.CORNER_RADIUS;
Blockly.BlockSvg.HAT_TOP_LEFT_CORNER_START = "m " + Blockly.BlockSvg.HAT_CORNER_RADIUS + ",0";
Blockly.BlockSvg.HAT_TOP_LEFT_CORNER = "A " + Blockly.BlockSvg.HAT_CORNER_RADIUS + "," + Blockly.BlockSvg.HAT_CORNER_RADIUS + " 0 0,0 0," + Blockly.BlockSvg.HAT_CORNER_RADIUS;
...
Blockly.BlockSvg.FIELD_TEXTINPUT_EXPAND_PAST_TRUNCATION = !0;
Blockly.BlockSvg.FIELD_TEXTINPUT_ANIMATE_POSITIONING = !0;


Blockly.BlockSvg.metricsAreEquivalent_ = function(a, b) {
    return a.statement != b.statement || a.imageField != b.imageField || a.height != b.height || a.width != b.width || a.bayHeight != b.bayHeight || a.bayWidth != b.bayWidth || a.fieldRadius != b.fieldRadius || a.startHat != b.startHat ? !1 : !0
}
;
Blockly.BlockSvg.prototype.connectionUiEffect = function() {
    this.workspace.playAudio("click")
...
Blockly.BlockSvg.prototype.updateColour = function() {
    var a;
    this.isGlowing_ ? this.getColourSecondary() : this.getColour();
    a = this.getColourTertiary();
    this.svgPath_.setAttribute("stroke", a);
    a = this.isGlowingBlock_ ? this.getColourSecondary() : this.getColour();
...
Blockly.BlockSvg.prototype.highlightForReplacement = function(a) {
    a ? (this.svgPath_.setAttribute("filter", "url(#blocklyReplacementGlowFilter)"),
    this.svgGroup_.classList.add("blocklyReplaceable")) : (this.svgPath_.removeAttribute("filter"),
    this.svgGroup_.classList.remove("blocklyReplaceable"))
}
;
Blockly.BlockSvg.prototype.getHeightWidth = function(a) {
    var b = this.height
      , c = this.width;
    !a && this.getFieldShadowBlock_() && (b += Blockly.BlockSvg.FIELD_Y_OFFSET,
    b += Blockly.BlockSvg.FIELD_HEIGHT);
    var d = this.getNextBlock();
    d && (a = d.getHeightWidth(a),
    c += a.width,
    c -= Blockly.BlockSvg.NOTCH_WIDTH,
...
Blockly.BlockSvg.prototype.render = function(a) {
    Blockly.Field.startCache();
    this.rendered = !0;
    var b = this.renderingMetrics_
      , c = this.renderCompute_();
    b && Blockly.BlockSvg.metricsAreEquivalent_(b, c) ? (c.statement && c.statement.connection && c.statement.targetConnection && c.statement.connection.tighten_(),
    this.nextConnection && this.nextConnection.targetConnection && this.nextConnection.tighten_()) : (this.height = c.height,
    this.width = c.width,
    this.renderDraw_(c),
    this.renderClassify_(c),
...
Blockly.BlockSvg.prototype.renderCompute_ = function() {
    for (var a = {
        ...
        startHat: !1,
        endCap: !1
    }, b = 0, c; c = this.inputList[b]; b++) {
        if (c.type == Blockly.NEXT_STATEMENT) {
            a.statement = c;
            a.bayHeight = Blockly.BlockSvg.MIN_BLOCK_Y;
            a.bayWidth = Blockly.BlockSvg.MIN_BLOCK_X;
            if (c.connection && c.connection.targetConnection) {
                var d = c.connection.targetBlock()
                  , d = d.getHeightWidth(!0);
                a.bayHeight = Math.max(a.bayHeight, d.height);
                a.bayWidth = Math.max(a.bayWidth, d.width)
            }
            (d = c.connection.targetBlock()) && !d.lastConnectionInStack() ? a.bayNotchAtRight = !1 : a.bayWidth -= Blockly.BlockSvg.NOTCH_WIDTH
        }
        for (var d = 0, e; e = c.fieldRow[d]; d++)
            e instanceof Blockly.FieldImage && (a.imageField = e),
            e instanceof Blockly.FieldIconMenu && (a.iconMenu = e),
            a.fieldRadius = e instanceof Blockly.FieldTextInput ? e.getBorderRadius() : Blockly.BlockSvg.FIELD_DEFAULT_CORNER_RADIUS
    }
    this.nextConnection && !this.previousConnection && (a.startHat = !0);
    this.nextConnection || !this.previousConnection || this.outputConnection || a.statement || (a.endCap = !0);
    a.iconMenu && (b = a.iconMenu.getSrcForValue(a.iconMenu.getValue())) && a.iconMenu.setParentFieldImage(b);
    a.width = 2 * Blockly.BlockSvg.SEP_SPACE_X + Blockly.BlockSvg.IMAGE_FIELD_WIDTH;
    a.height = 2 * Blockly.BlockSvg.SEP_SPACE_Y + Blockly.BlockSvg.IMAGE_FIELD_HEIGHT;
    this.outputConnection && (a.height = Blockly.BlockSvg.FIELD_HEIGHT,
    a.width = Blockly.BlockSvg.FIELD_WIDTH);
    a.statement && (a.width += a.bayWidth + 4 * Blockly.BlockSvg.CORNER_RADIUS + 2 * Blockly.BlockSvg.GRID_UNIT,
    a.height = a.bayHeight + Blockly.BlockSvg.STATEMENT_BLOCK_SPACE);
    a.startHat && (a.width += 1 * Blockly.BlockSvg.GRID_UNIT);
    a.endCap && (a.width += 1 * Blockly.BlockSvg.GRID_UNIT);
    return a
}
;
Blockly.BlockSvg.prototype.renderDraw_ = function(a) {
    var b = this.getRelativeToSurfaceXY()
      , c = [];
    this.renderDrawLeft_(c, b, a);
    this.renderDrawBottom_(c, b, a);
    this.renderDrawRight_(c, b, a);
    this.renderDrawTop_(c, b, a);
    b = c.join(" ");
    this.svgPath_.setAttribute("d", b);
    this.RTL && this.svgPath_.setAttribute("transform", "scale(-1 1)");
    if (a.imageField) {
        var b = a.imageField.getSvgRoot()
          , c = a.imageField.getSize()
          , d = a.width - c.width - Blockly.BlockSvg.SEP_SPACE_X / 1.5
          , e = a.height - c.height - Blockly.BlockSvg.SEP_SPACE_Y;
        a.endCap && (d -= Blockly.BlockSvg.GRID_UNIT);
        var f = "scale(1 1)";
        this.RTL && (a.imageField.getFlipRTL() ? (f = "scale(-1 1)",
        d = -a.width + c.width + Blockly.BlockSvg.SEP_SPACE_X / 1.5) : d = -a.width + Blockly.BlockSvg.SEP_SPACE_X / 1.5);
        b && (this.isInsertionMarker() && b.setAttribute("display", "none"),
        b.setAttribute("transform", "translate(" + d + "," + e + ") " + f))
    }
    this.getFieldShadowBlock_() && (b = this.getFieldShadowBlock_().getSvgRoot(),
    c = Blockly.BlockSvg.NOTCH_WIDTH + (a.bayWidth ? 2 * Blockly.BlockSvg.GRID_UNIT + 2 * Blockly.BlockSvg.NOTCH_WIDTH : 0) + a.bayWidth,
    a.startHat && (c += 2 * Blockly.BlockSvg.GRID_UNIT),
    this.RTL && (c = -c),
    b.setAttribute("transform", "translate(" + c + "," + (a.height + Blockly.BlockSvg.FIELD_Y_OFFSET) + ")"))
}
;
Blockly.BlockSvg.prototype.renderClassify_ = function(a) {
    var b = [];
    this.isShadow_ ? b.push("argument") : (a.statement && b.push("c-block"),
    a.startHat ? b.push("hat") : a.statement || b.push("stack"),
    this.nextConnection || b.push("end"));
    this.svgGroup_.setAttribute("data-shapes", b.join(" "));
...
Blockly.BlockSvg.prototype.renderDrawLeft_ = function(a, b, c) {
    c.startHat ? (a.push(Blockly.BlockSvg.HAT_TOP_LEFT_CORNER_START),
    a.push(Blockly.BlockSvg.HAT_TOP_LEFT_CORNER)) : this.previousConnection ? (a.push(Blockly.BlockSvg.TOP_LEFT_CORNER_START),
    a.push(Blockly.BlockSvg.TOP_LEFT_CORNER),
    a.push("V", c.height - Blockly.BlockSvg.CORNER_RADIUS - Blockly.BlockSvg.SEP_SPACE_Y - Blockly.BlockSvg.NOTCH_HEIGHT),
    a.push(Blockly.BlockSvg.NOTCH_PATH_DOWN),
    this.previousConnection.moveTo(b.x, b.y + c.height - 2 * Blockly.BlockSvg.CORNER_RADIUS),
    a.push("V", c.height - Blockly.BlockSvg.CORNER_RADIUS)) : (a.push("m", c.fieldRadius + ",0"),
    a.push("A", c.fieldRadius + "," + c.fieldRadius, "0", "0,0", "0," + c.fieldRadius),
    a.push("V", c.height - c.fieldRadius))
}
;
Blockly.BlockSvg.prototype.renderDrawBottom_ = function(a, b, c) {
    c.startHat ? a.push("a", Blockly.BlockSvg.HAT_CORNER_RADIUS + "," + Blockly.BlockSvg.HAT_CORNER_RADIUS + " 0 0,0 " + Blockly.BlockSvg.HAT_CORNER_RADIUS + "," + Blockly.BlockSvg.HAT_CORNER_RADIUS) : this.previousConnection ? a.push("a", Blockly.BlockSvg.CORNER_RADIUS + "," + Blockly.BlockSvg.CORNER_RADIUS + " 0 0,0 " + Blockly.BlockSvg.CORNER_RADIUS + "," + Blockly.BlockSvg.CORNER_RADIUS) : a.push("a", c.fieldRadius + "," + c.fieldRadius, "0", "0,0", c.fieldRadius + "," + c.fieldRadius);
    if (c.statement) {
        a.push("h", 4 * Blockly.BlockSvg.GRID_UNIT);
        a.push("a", Blockly.BlockSvg.CORNER_RADIUS + "," + Blockly.BlockSvg.CORNER_RADIUS + " 0 0,0 " + Blockly.BlockSvg.CORNER_RADIUS + ",-" + Blockly.BlockSvg.CORNER_RADIUS);
        a.push("v", -2.5 * Blockly.BlockSvg.GRID_UNIT);
        a.push(Blockly.BlockSvg.NOTCH_PATH_UP);
        a.push("v", -c.bayHeight + 3 * Blockly.BlockSvg.CORNER_RADIUS + Blockly.BlockSvg.NOTCH_HEIGHT + 2 * Blockly.BlockSvg.GRID_UNIT);
        ...
        a.push("h", c.bayWidth - 2 * Blockly.BlockSvg.CORNER_RADIUS);
        a.push("a", Blockly.BlockSvg.CORNER_RADIUS + "," + Blockly.BlockSvg.CORNER_RADIUS + " 0 0,1 " + Blockly.BlockSvg.CORNER_RADIUS + "," + Blockly.BlockSvg.CORNER_RADIUS);
        c.bayNotchAtRight && (a.push("v", c.bayHeight - 3 * Blockly.BlockSvg.CORNER_RADIUS - Blockly.BlockSvg.NOTCH_HEIGHT - 2 * Blockly.BlockSvg.GRID_UNIT),
        a.push(Blockly.BlockSvg.NOTCH_PATH_DOWN));
        a.push("V", c.bayHeight + 2 * Blockly.BlockSvg.GRID_UNIT);
        a.push("a", Blockly.BlockSvg.CORNER_RADIUS + "," + Blockly.BlockSvg.CORNER_RADIUS + " 0 0,0 " + Blockly.BlockSvg.CORNER_RADIUS + "," + Blockly.BlockSvg.CORNER_RADIUS);
        var d = b.x + 2 * Blockly.BlockSvg.CORNER_RADIUS + 4 * Blockly.BlockSvg.GRID_UNIT;
        this.RTL && (d = b.x - 2 * Blockly.BlockSvg.CORNER_RADIUS - 4 * Blockly.BlockSvg.GRID_UNIT);
        c.statement.connection.moveTo(d, b.y + c.height - 2 * Blockly.BlockSvg.CORNER_RADIUS);
        c.statement.connection.targetConnection && c.statement.connection.tighten_()
    }
    this.isShadow() ? a.push("H", c.width - c.fieldRadius) : a.push("H", c.width - Blockly.BlockSvg.CORNER_RADIUS)
}
;
Blockly.BlockSvg.prototype.renderDrawRight_ = function(a, b, c) {
    this.isShadow() ? (a.push("a", c.fieldRadius + "," + c.fieldRadius, "0", "0,0", c.fieldRadius + "," + -1 * c.fieldRadius),
    a.push("v", -1 * (c.height - 2 * c.fieldRadius))) : (a.push("a", Blockly.BlockSvg.CORNER_RADIUS + "," + Blockly.BlockSvg.CORNER_RADIUS + " 0 0,0 " + Blockly.BlockSvg.CORNER_RADIUS + ",-" + Blockly.BlockSvg.CORNER_RADIUS),
    a.push("v", -2.5 * Blockly.BlockSvg.GRID_UNIT));
    this.nextConnection ? (a.push(Blockly.BlockSvg.NOTCH_PATH_UP),
    this.width += Blockly.BlockSvg.NOTCH_WIDTH,
    this.nextConnection.moveTo(this.RTL ? b.x - c.width : b.x + c.width, b.y + c.height - 2 * Blockly.BlockSvg.CORNER_RADIUS),
    this.nextConnection.targetConnection && this.nextConnection.tighten_(),
    a.push("V", Blockly.BlockSvg.CORNER_RADIUS)) : this.isShadow() || a.push("V", Blockly.BlockSvg.CORNER_RADIUS)
}
;
Blockly.BlockSvg.prototype.renderDrawTop_ = function(a, b, c) {
    this.isShadow() ? a.push("a", c.fieldRadius + "," + c.fieldRadius, "0", "0,0", "-" + c.fieldRadius + ",-" + c.fieldRadius) : a.push("a", Blockly.BlockSvg.CORNER_RADIUS + "," + Blockly.BlockSvg.CORNER_RADIUS + " 0 0,0 -" + Blockly.BlockSvg.CORNER_RADIUS + ",-" + Blockly.BlockSvg.CORNER_RADIUS);
    a.push("z")
}
;
Blockly.BlockSvg.prototype.getFieldShadowBlock_ = function() {
    for (var a = 0, b; b = this.childBlocks_[a]; a++)
        if (b.isShadow())
            return b;
    return null
}
...
Blockly.BlockSvg.prototype.positionNewBlock = function(a, b, c) {
    if (b.type == Blockly.NEXT_STATEMENT) {
        var d = c.x_ - b.x_
          , e = c.y_ - b.y_;
        b == a.getFirstStatementConnection() && (e -= c.sourceBlock_.getHeightWidth(!0).height - Blockly.BlockSvg.MIN_BLOCK_Y);
        a.moveBy(d, e)
    }
}
;
...
;
Blockly.bindEvent_ = function(a, b, c, d, e) {
    var f = !1
      , g = function(a) {
        var b = !e;
...
Blockly.getRelativeXY_ = function(a) {
    ...
    if (c = a.getAttribute("transform"))
        if (c = c.match(Blockly.getRelativeXY_.XY_REGEXP_))
            b.x += parseFloat(c[1]),
            c[3] && (b.y += parseFloat(c[3]));
    (a = a.getAttribute("style")) && -1 < a.indexOf("translate3d") && (a = a.match(Blockly.getRelativeXY_.XY_3D_REGEXP_)) && (b.x += parseFloat(a[1]),
    a[3] && (b.y += parseFloat(a[3])));
    return b
}
;
Blockly.getRelativeXY_.XY_REGEXP_ = /translate\(\s*([-+\d.e]+)([ ,]\s*([-+\d.e]+)\s*\))?/;
Blockly.getRelativeXY_.XY_3D_REGEXP_ = /transform:\s*translate3d\(\s*([-+\d.e]+)px([ ,]\s*([-+\d.e]+)\s*)px([ ,]\s*([-+\d.e]+)\s*)px\)?/;
...
Blockly.is3dSupported = function() {
    if (null !== Blockly.cache3dSupported_)
    ...
    Blockly.cache3dSupported_ = void 0 !== b && 0 < b.length && "none" !== b;
    return Blockly.cache3dSupported_
}
;
...
;
Blockly.measureText = function(a, b, c, d) {
    var e = document.createElement("canvas").getContext("2d");
    e.font = c + " " + a + " " + b;
    return e.measureText(d).width
}
;
Blockly.encodeEntities = function(a) {
...
Blockly.FieldTextInput.ANIMATION_TIME = .25;
...
Blockly.FieldTextInput.prototype.init = function() {
    ...
    this.sourceBlock_.isShadow() || (this.box_ = Blockly.createSvgElement("rect", {
 ...
 * 触发弹窗
 */
function getinput(numb){
    numbdelete();
   // setTimeout("numbdelete2();textclick("+numb+")",100);
    textclick(numb);
    $('.button.javascript').css({"bottom":"10px"});
};
/**
 * 生成弹窗
 ...
 */
function numbinput(){
      // $('.blocklyWidgetDiv.fieldTextInput').css({"visibility": "visible"});   
      // Blockly.Xml.workspaceToDom(Blockly.mainWorkspace)   //获取xml
      var numb=parseFloat($('#numbox .swiper-slide-active').text());
      if($('.blocklyHtmlInput').length>0){
          $('.blocklyHtmlInput').val(numb);
          Blockly.WidgetDiv.hide();
      }else{
        var option=$('#numbox .swiper-slide-active').index();
        Blockly.selected.getFieldShadowBlock_().inputList[0].fieldRow[0].setValue(20) //important    rongwu 
        Blockly.DropDownDiv.hide();
      }
       $('#numbox').remove();
       $('.dwts').remove();
...
Blockly.FieldTextInput.prototype.showEditor_ = function(a, b, c, d) {
    ...
    if(strings.indexOf('motion_function')>-1){
          //函数模块显示      
          if($('body').hasClass('app-intro')){
            
           }else{
                  $('#funceditor .injectionDiv').remove();
                  setmain2();
                  var xml=$('#'+Blockly.selected.type).html();
                  var name=$('#'+Blockly.selected.type).atttr('name');
                  var intro=$('#'+Blockly.selected.type).atttr('intro');
                  funcshow(xml);
          }
    }else{
    ...
    //f.setAttribute("spellcheck", this.spellcheck_);   add by rongw
    e && f.setAttribute("readonly", "true");
    Blockly.FieldTextInput.htmlInput_ = f;
    b.appendChild(f);
    c && (this.sourceBlock_.RTL ? f.style.paddingLeft = this.arrowSize_ + Blockly.BlockSvg.DROPDOWN_ARROW_PADDING + "px" : f.style.paddingRight = this.arrowSize_ + Blockly.BlockSvg.DROPDOWN_ARROW_PADDING + "px",
    c = goog.dom.createDom("IMG", "blocklyTextDropDownArrow"),
    c.setAttribute("src", Blockly.mainWorkspace.options.pathToMedia + "dropdown-arrow-dark.svg"),
    c.style.width = this.arrowSize_ + "px",
    c.style.height = this.arrowSize_ + "px",
    c.style.top = this.arrowY_ + "px",
    c.style.cursor = "pointer",
    this.sourceBlock_.RTL ? c.style.left = "11px" : c.style.right = "11px",
    d && (f.dropDownArrowMouseWrapper_ = Blockly.bindEvent_(c, "mousedown", this, d)),
    b.appendChild(c));
    ...
    f.onKeyDownWrapper_ = Blockly.bindEvent_(f, "keydown", this, this.onHtmlInputKeyDown_);
    f.onKeyUpWrapper_ = Blockly.bindEvent_(f, "keyup", this, this.onHtmlInputChange_);
    f.onKeyPressWrapper_ = Blockly.bindEvent_(f, "keypress", this, this.onHtmlInputChange_);
...
//add by rongw
    $('.blocklyWidgetDiv.fieldTextInput').css({"visibility": "hidden"});
    if(!$('body').hasClass('app-intro')){
        if(Blockly.selected.type!='function_name' && Blockly.FieldTextInput.htmlInput_.defaultValue!='function name' && this.sourceBlock_.isInFlyout==false){
            //console.log($('.blocklyHtmlInput').val());
            if($('.blocklyHtmlInput').length>0){
                setTimeout("getinput($('.blocklyHtmlInput').val())","200"); 
        ...
        if (goog.userAgent.GECKO)
            ...
            32 > b || 127 == b)
                ...
                if (a.metaKey || a.ctrlKey)
                    c = -1 < Blockly.FieldTextInput.GECKO_KEYCODE_WHITELIST.indexOf(b)
            }
        else
            b = a.keyCode;
        b = String.fromCharCode(b);
        if (!c && !this.restrictor_.test(b) && a.preventDefault) {
            a.preventDefault();
            return
        }
    }
...
Blockly.FieldTextInput.prototype.resizeEditor_ = function() {
    var a = this.sourceBlock_.workspace.scale, b = Blockly.WidgetDiv.DIV, c;
    Blockly.BlockSvg.FIELD_TEXTINPUT_EXPAND_PAST_TRUNCATION ? (c = Blockly.measureText(Blockly.FieldTextInput.htmlInput_.style.fontSize, Blockly.FieldTextInput.htmlInput_.style.fontFamily, Blockly.FieldTextInput.htmlInput_.style.fontWeight, Blockly.FieldTextInput.htmlInput_.value),
    c += Blockly.FieldTextInput.TEXT_MEASURE_PADDING_MAGIC,
    c *= a) : c = this.sourceBlock_.getHeightWidth().width * a;
    ...
    b.style.height = Blockly.BlockSvg.FIELD_HEIGHT_MAX_EDIT + 1 + "px";
    ...
    d.x -= a / 2;
    d.y -= a / 2;
    this.sourceBlock_.RTL && (d.x += c,
    d.x -= b.offsetWidth * a,
    d.x += 1 * a);
    d.y += 1 * a;
    goog.userAgent.GECKO && Blockly.WidgetDiv.DIV.style.top && (d.x += 2 * a,
    d.y += 1 * a);
...
Blockly.FieldTextInput.prototype.widgetDispose_ = function() {
    ...
    return function() {
        var b = Blockly.WidgetDiv.DIV
          ...
          , d = c.value;
        if (a.sourceBlock_)
            if (d = a.callValidator(d),
            null === d)
                d = c.defaultValue;
            else if (a.onFinishEditing_)
                a.onFinishEditing_(d);
        a.setValue(d);
        ...
        b.style.boxShadow = "";
        a.sourceBlock_ && (c = a.sourceBlock_.getHeightWidth(),
        b.style.width = c.width + 1 + "px",
        b.style.height = c.height + 1 + "px");
...
Blockly.FieldAngle.prototype.showEditor_ = function() {
    ...
    this.clickWrapper_ = Blockly.bindEvent_(a, "click", this, function() {
        Blockly.WidgetDiv.hide();
        Blockly.DropDownDiv.hide()
    });
...
Blockly.FieldDropdown.prototype.showEditor_ = function() {
    if(this.sourceBlock_.isInFlyout==false){ //add by rongw
    ...
    for (var d = this.getOptions_(), e = 0; e < d.length; e++) {
        var f = d[e][1]
          , g = new goog.ui.MenuItem(d[e][0]);
        g.setRightToLeft(this.sourceBlock_.RTL);
        g.setValue(f);
        g.setCheckable(!0);
        c.addChild(g, !0);
        f = f == this.value_;
        g.setChecked(f);
        f && (this.selectedItem = g)
    }
    goog.events.listen(c, goog.ui.Component.EventType.ACTION, function(a) {
        if (a = a.target)
            a = a.getValue(),
    ...
    c.getHandler().listen(c.getElement(), goog.events.EventType.TOUCHSTART, function(a) {
        this.getOwnerControl(a.target).handleMouseDown(a)
    });
    c.getHandler().listen(c.getElement(), goog.events.EventType.TOUCHEND, function(a) {
        this.getOwnerControl(a.target).performActionInternal(a)
    });
    c.render(a);
    a = c.getElement();
    Blockly.addClass_(a, "blocklyDropdownMenu");
    ...
    g *= e;
    d *= e;
    e = goog.style.getPageOffset(this.fieldGroup_);
    g = e.x + g / 2;
    d = e.y + d;
    e = e.y;
    ...
    a.focus();
    this.disableColourChange_ || (this.sourceBlock_.isShadow() ? (this.savedPrimary_ = this.sourceBlock_.getColour(),
    this.sourceBlock_.setColour(this.sourceBlock_.getColourTertiary(), this.sourceBlock_.getColourSecondary(), this.sourceBlock_.getColourTertiary())) : this.box_ && this.box_.setAttribute("fill", this.sourceBlock_.getColourTertiary()))
}
}
;
Blockly.FieldDropdown.prototype.onHide = function() {
    this.dropDownOpen_ = !1;
    this.disableColourChange_ || (this.sourceBlock_.isShadow() ? this.sourceBlock_.setColour(this.savedPrimary_, this.sourceBlock_.getColourSecondary(), this.sourceBlock_.getColourTertiary()) : this.box_ && this.box_.setAttribute("fill", this.sourceBlock_.getColour()))
}
;
...
Blockly.FieldDropdown.prototype.setText = function(a) {
    ...
    this.updateTextNode_(),
    this.textElement_ && (this.textElement_.setAttribute("class", this.textElement_.getAttribute("class") + " blocklyDropdownText"),
...
Blockly.FieldIconMenu.prototype.setValue = function(a) {
    null !== a && a !== this.value_ && (this.sourceBlock_ && Blockly.Events.isEnabled() && Blockly.Events.fire(new Blockly.Events.Change(this.sourceBlock_,"field",this.name,this.value_,a)),
    this.value_ = a,
...
Blockly.FieldIconMenu.prototype.setParentFieldImage = function(a) {
    if (this.sourceBlock_ && this.sourceBlock_.parentBlock_)
        for (var b = this.sourceBlock_.parentBlock_, c = 0, d; d = b.inputList[c]; c++)
            for (var e = 0, f; f = d.fieldRow[e]; e++)
                if (f instanceof Blockly.FieldImage) {
                    f.setValue(a);
                    return
                }
}
...
Blockly.FieldIconMenu.prototype.getSrcForValue = function(a) {
    for (var b = 0, c; c = this.icons_[b]; b++)
        if (c.value === a)
            return c.src
}
;
    ...
    if (!Blockly.DropDownDiv.hideIfOwner(this)) {
        ...
        for (var b = 0, c; c = this.icons_[b]; b++) {
            if ("placeholder" == c.type) {
                ...
                c.value == this.getValue() && (e = this.sourceBlock_.getColourTertiary(),
                ...
                Blockly.bindEvent_(d, "mousedown", d, function(a) {
                    this.setAttribute("class", "blocklyDropDownButton blocklyDropDownButtonHover");
                    a.preventDefault()
                });
                Blockly.bindEvent_(d, "mouseover", d, function() {
                    this.setAttribute("class", "blocklyDropDownButton blocklyDropDownButtonHover");
                    a.setAttribute("aria-activedescendant", this.id)
                });
                Blockly.bindEvent_(d, "mouseout", d, function() {
...
;
Blockly.FieldTextDropdown = function(a, b, c, d) {
    this.menuGenerator_ = b;
...
Blockly.FieldTextDropdown.prototype.init = function() {
    ...
    this.arrow_ || (this.arrowSize_ = 12,
    this.arrowX_ = 0,
    this.arrowY_ = 11,
    this.arrow_ = Blockly.createSvgElement("image", {
        height: this.arrowSize_ + "px",
...
Blockly.FieldTextDropdown.prototype.showEditor_ = function() {
    this.dropDownOpen_ || Blockly.FieldTextDropdown.superClass_.showEditor_.call(this, null, null, !0, function() {
        Blockly.WidgetDiv.hide();
        this.showDropdown_();
        Blockly.Touch.clearTouchIdentifier()
    })
...
Blockly.FieldNumber.prototype.showEditor_ = function() {
    ...
    if(ios==1 || ios=='1' ){
        var a = goog.userAgent.MOBILE || goog.userAgent.ANDROID || goog.userAgent.IPAD;  //add by rongw 2017.06.02
        
              Blockly.FieldNumber.superClass_.showEditor_.call(this, !1, a);
    }
    else if(android==1 || android=='1' ){
        ...
        var a = goog.userAgent.MOBILE || goog.userAgent.ANDROID || goog.userAgent.IPAD;  //add by rongw 2017.06.02
          ...
          var a = false;
          goog.userAgent.MOBILE || goog.userAgent.ANDROID || goog.userAgent.IPAD;  //add by rongw
...
Blockly.FieldNumber.prototype.showNumPad_ = function() {
    ...
    a.setAttribute("role", "menu");
    a.setAttribute("aria-haspopup", "true");
    for (var b = Blockly.FieldNumber.NUMPAD_BUTTONS, c = 0, d; d = b[c]; c++) {
        var e = document.createElement("button");
        e.setAttribute("role", "menuitem");
        e.setAttribute("class", "blocklyNumPadButton");
        e.title = d;
        e.innerHTML = d;
        Blockly.bindEvent_(e, "mousedown", e, Blockly.FieldNumber.numPadButtonTouch_);
        "." != d || this.decimalAllowed_ || e.setAttribute("style", "visibility: hidden");
        a.appendChild(e)
    }
    ...
    b.appendChild(c);
    Blockly.bindEvent_(b, "mousedown", null, Blockly.FieldNumber.numPadEraseButtonTouch_);
    a.appendChild(b);
    ...
    a = c.x + d.width / 2;
    d = c.y + d.height + Blockly.FieldNumber.DROPDOWN_Y_PADDING;
    b = c.y - Blockly.BlockSvg.MIN_BLOCK_Y * b - Blockly.BlockSvg.FIELD_Y_OFFSET * b;
    Blockly.DropDownDiv.setBoundsElement(this.sourceBlock_.workspace.getParentSvg().parentNode);
    Blockly.DropDownDiv.show(this, a, d, a, b, this.onHide_.bind(this))
}
;
...
Blockly.FieldNumber.numPadEraseButtonTouch_ = function() {
    ...
    Blockly.FieldTextInput.htmlInput_.value = d;
    Blockly.FieldNumber.superClass_.resizeEditor_.call(Blockly.FieldNumber.activeField_);
    Blockly.FieldTextInput.htmlInput_.setSelectionRange(d.length, d.length);
    Blockly.FieldTextInput.htmlInput_.scrollLeft = Blockly.FieldTextInput.htmlInput_.scrollWidth;
    Blockly.FieldNumber.activeField_.validate_()
}
;
...
Blockly.Variables.allUsedVariables = function(a) {
    ...
    if (a instanceof Blockly.Block)
        b = a.getDescendants();
    else if (a instanceof Blockly.Workspace || a instanceof Blockly.WorkspaceSvg)
        b = a.getAllBlocks();
    else
...
Blockly.Variables.flyoutCategory = function(a) {
    ...
    for (c = 0; c < a.length; c++)
        if (Blockly.Blocks.data_variable) {
            ...
            d.setAttribute("type", "data_variable");
            d.setAttribute("gap", 8);
            d.appendChild(Blockly.Variables.createVariableDom_(a[c]));
            b.push(d)
        }
    1 < b.length && (b[b.length - 1].setAttribute("gap", 24),
    Blockly.Blocks.data_setvariableto && (d = goog.dom.createDom("block"),
    d.setAttribute("type", "data_setvariableto"),
    d.setAttribute("gap", 8),
    d.appendChild(Blockly.Variables.createVariableDom_(a[0])),
    d.appendChild(Blockly.Variables.createTextDom_()),
    b.push(d)),
    Blockly.Blocks.data_changevariableby && (d = goog.dom.createDom("block"),
    d.setAttribute("type", "data_changevariableby"),
    d.setAttribute("gap", 8),
    d.appendChild(Blockly.Variables.createVariableDom_(a[0])),
    d.appendChild(Blockly.Variables.createMathNumberDom_()),
    b.push(d)),
    Blockly.Blocks.data_showvariable && (d = goog.dom.createDom("block"),
    d.setAttribute("type", "data_showvariable"),
    d.setAttribute("gap", 8),
    d.appendChild(Blockly.Variables.createVariableDom_(a[0])),
    b.push(d)),
    Blockly.Blocks.data_hidevariable && (d = goog.dom.createDom("block"),
    d.setAttribute("type", "data_hidevariable"),
    d.appendChild(Blockly.Variables.createVariableDom_(a[0])),
    b.push(d)));
    return b
}
...
Blockly.Variables.createVariableDom_ = function(a) {
    var b = Blockly.Variables.createValueDom_("VARIABLE")
      , c = Blockly.Variables.createShadowDom_("data_variablemenu");
...
Blockly.FieldVariable.prototype.classValidator = function(a) {
    ...
    if (a == Blockly.Msg.DELETE_VARIABLE.replace("%1", this.getText()))
        return b.deleteVariable(this.getText()),
        null
}
;
...
Blockly.Generator.prototype.blockToCode = function(a) {
    ...
    if (goog.isString(b))
        return this.STATEMENT_PREFIX && (b = this.STATEMENT_PREFIX.replace(/%1/g, "'" + a.id + "'") + b),
        this.scrub_(a, b);
    if (null === b)
        return "";
...
Blockly.Generator.prototype.provideFunction_ = function(a, b) {
    if (!this.definitions_[a]) {
        ...
        for (var c = b.join("\n").replace(this.FUNCTION_NAME_PLACEHOLDER_REGEXP_, c), d; d != c; )
            d = c,
            c = c.replace(/^(( {2})*) {2}/gm, "$1" + this.INDENT);
...
Blockly.Procedures.flyoutCategory = function(a) {
    ...
    d.setAttribute("gap", 16),
    c.push(d));
    Blockly.Blocks.procedures_report && (d = goog.dom.createDom("block"),
    d.setAttribute("type", "procedures_report"),
    d.setAttribute("gap", 16),
    c.push(d));
...
Blockly.Flyout.prototype.show = function(a) {
    ...
    for (var b = [], c = [], d = this.permanentlyDisabled_.length = 0, e; e = a[d]; d++)
        if (e.tagName) {
            ...
            c.push(isNaN(e) ? g : e)) : "SEP" == e.tagName.toUpperCase() ? (e = parseInt(e.getAttribute("gap"), 10),
            !isNaN(e) && 0 < c.length ? c[c.length - 1] = e : c.push(g)) : "BUTTON" == f && (e = e.getAttribute("text"),
            e = new Blockly.FlyoutButton(this.workspace_,this.targetWorkspace_,e),
            b.push({
                type: "button",
                button: e
            }),
...
Blockly.Flyout.prototype.addBlockListeners_ = function(a, b, c) {
    this.autoClose ? (this.listeners_.push(Blockly.bindEvent_(a, "mousedown", null, this.createBlockFunc_(b))),
    this.listeners_.push(Blockly.bindEvent_(c, "mousedown", null, this.createBlockFunc_(b)))) : (this.listeners_.push(Blockly.bindEvent_(a, "mousedown", null, this.blockMouseDown_(b))),
...
Blockly.Flyout.prototype.blockMouseDown_ = function(a) {
    var b = this;
    return function(c) {
        Blockly.isRightButton(c) ? Blockly.Flyout.blockRightClick_(c, a) : (b.dragMode_ = Blockly.DRAG_NONE,
        Blockly.terminateDrag_(),
        Blockly.WidgetDiv.hide(!0),
        Blockly.DropDownDiv.hideWithoutAnimation(),
        Blockly.hideChaff(),
        ...
        Blockly.Flyout.onMouseMoveBlockWrapper_ = Blockly.bindEvent_(document, "mousemove", b, b.onMouseMoveBlock_));
        c.stopPropagation();
        c.preventDefault()
    }
}
...
Blockly.Flyout.prototype.onMouseDown_ = function(a) {
    ...
    Blockly.isRightButton(a) ? Blockly.Touch.clearTouchIdentifier() : (Blockly.WidgetDiv.hide(!0),
    Blockly.DropDownDiv.hideWithoutAnimation(),
    Blockly.hideChaff(!0),
    this.dragMode_ = Blockly.DRAG_FREE,
...
;
Blockly.Flyout.prototype.onMouseUp_ = function() {
    ...
    Blockly.Events.fire(new Blockly.Events.Ui(Blockly.Flyout.startBlock_,"stackclick",void 0,void 0))));
    Blockly.terminateDrag_()
}
;
...
Blockly.Flyout.prototype.onMouseMoveBlock_ = function(a) {
    if (!("mousemove" == a.type && 1 >= a.clientX && 0 == a.clientY && 0 == a.button)) {
        Blockly.Css.setCursor(Blockly.Css.Cursor.CLOSED);
        var b = this.determineDragIntention_(a.clientX - Blockly.Flyout.startDownEvent_.clientX, a.clientY - Blockly.Flyout.startDownEvent_.clientY);
        Blockly.longStop_();
        if (b)
            setTimeout("if(!$('#blocklyDiv').hasClass('simple')){$('.blocklyFlyout').hide();$('.scratchCategoryMenuItem ').removeClass('categorySelected')}",'100'),   //add by rongw 掩藏flyout
...
Blockly.HorizontalFlyout.prototype.setMetrics_ = function(a) {
    var b = this.getMetrics_();
    b && (goog.isNumber(a.x) && (this.workspace_.scrollX = -b.contentWidth * a.x),
...
Blockly.HorizontalFlyout.prototype.position = function() {
    ...
    if (this.isVisible()) {
        ...
        var a = this.targetWorkspace_.getMetrics();
        if (a) {
            this.width_ = this.workspace_.getMetrics().contentWidth;//9.01   add 自适应
            if (this.parentToolbox_){
               if(!$('#blocklyDiv').hasClass('simple') || $('body').hasClass('web-admin')){
                    var b = this.parentToolbox_.HtmlDiv.offsetLeft +141;   //add by rongw
                }else{
                    var b = this.parentToolbox_.HtmlDiv.offsetLeft +141;   //add by rongw
                }
                  c = this.parentToolbox_.HtmlDiv.offsetTop ; 
                  //+ this.parentToolbox_.getHeight();  add by rongw
            }
            else{
                b = this.toolboxPosition_ == Blockly.TOOLBOX_AT_RIGHT ? a.viewWidth - this.width_ : 0;
            }

                       var url=window.location.href;
                       var ios=0;var android=0;
                       ios=getUrlParam('ios');
                       android=getUrlParam('android');
                       
              if($('body').hasClass('web-admin') && !$('#blocklyDiv').hasClass('simple')){
                               c = 0;
                  ...
                  this.setBackgroundPath_(this.width_, this.height_);    
                                ...
                                this.height_=this.height_; //add by rongw msg的高度\  
                                if($('body').hasClass('web-admin2') ){ 
                                   this.svgGroup_.setAttribute("height", this.height_);
                                   if( !$('#blocklyDiv').hasClass('simple')){
                                       this.svgGroup_.setAttribute("width", '140');
                                   }else{
                                       $('#prev').hide();
                                       this.svgGroup_.setAttribute("width", this.width_);
                                   }
                                }else{
                                    if($('body').hasClass('index2')){
                                        $('#prev').hide();
                                        this.svgGroup_.setAttribute("height", this.height_);
                                        this.svgGroup_.setAttribute("width", '140');
                                        this.setBackgroundPath_(140, this.height_);
                                    }else{
                                        $('#prev').hide();
                                        this.svgGroup_.setAttribute("height", this.height_-44);
                                        this.svgGroup_.setAttribute("width", this.width_);
                                        this.setBackgroundPath_(140, this.height_-44);
                                    }
...
;
Blockly.HorizontalFlyout.prototype.onMouseMove_ = function(a) {
    var b = this.getMetrics_();
    if (!(0 > b.contentWidth - b.viewWidth)) {
        var c = a.clientX - this.startDragMouseX_;
        this.startDragMouseX_ = a.clientX;
...
;
Blockly.HorizontalFlyout.prototype.isDragTowardWorkspace_ = function(a, b) {
    ...
    if (this.height_ != b) {
        for (c = 0; d = a[c]; c++) {
            ...
            if (d.flyoutRect_) {
                ...
                d.flyoutRect_.setAttribute("x", this.RTL ? f.x - e.width : f.x);
...
Blockly.VerticalFlyout.prototype.autoClose = !1;
...
Blockly.VerticalFlyout.prototype.CHECKBOX_SPACE_X = Blockly.VerticalFlyout.prototype.CHECKBOX_SIZE + 2 * Blockly.VerticalFlyout.prototype.CHECKBOX_MARGIN;
Blockly.VerticalFlyout.prototype.init = function(a) {
...
Blockly.VerticalFlyout.prototype.setMetrics_ = function(a) {
    var b = this.getMetrics_();
    b && (goog.isNumber(a.y) && (this.workspace_.scrollY = -b.contentHeight * a.y),
    this.workspace_.translate(this.workspace_.scrollX + b.absoluteLeft, this.workspace_.scrollY + b.absoluteTop),
    this.clipRect_.setAttribute("height", b.viewHeight + "px"),
    this.clipRect_.setAttribute("width", b.viewWidth + "px"))
}
;
Blockly.VerticalFlyout.prototype.position = function() {
    if (this.isVisible()) {
        ...
        //           , c = this.parentToolbox_.HtmlDiv.offsetTop + this.parentToolbox_.getHeight();
        //     else
        //         b = this.toolboxPosition_ == Blockly.TOOLBOX_AT_RIGHT ? a.viewWidth - this.width_ : 0,
        //         c = 0;
        //     this.svgGroup_.setAttribute("transform", "translate(" + b + "," + c + ")");
        ...
        if (a) {
            ...
            this.scrollbar_ && this.scrollbar_.resize();
            this.svgGroup_.style.opacity = 1;
            this.svgGroup_.style.transform = "translate(" + b + "," + c + ")";
        }
    }
}
;
...
Blockly.VerticalFlyout.prototype.clearOldBlocks_ = function() {
    ...
    for (var a = 0, b; b = this.checkboxes_[a]; a++)
        b.block.flyoutCheckbox = null,
...
Blockly.VerticalFlyout.prototype.layout_ = function(a, b) {
    for (var c = this.MARGIN, d = this.getWidth() / this.workspace_.scale, e = c, f = 0, g; g = a[f]; f++)
        if ("block" == g.type) {
            g = g.block;
            for (var h = g.getDescendants(), k = 0, l; l = h[k]; k++)
                l.isInFlyout = !0;
            h = g.getSvgRoot();
            k = g.getHeightWidth();
            ...
            var m = d - this.MARGIN;
            l = this.RTL ? m - l : c;
            g.hasCheckboxInFlyout() && (this.createCheckbox_(g, c, e, k),
            l = this.RTL ? l - (this.CHECKBOX_SIZE + this.CHECKBOX_MARGIN) : l + (this.CHECKBOX_SIZE + this.CHECKBOX_MARGIN));
            g.moveBy(l, e + (g.startHat_ ? Blockly.BlockSvg.START_HAT_HEIGHT : 0));
            l = this.createRect_(g, l, e, k, f);
            this.addBlockListeners_(h, g, l);
            e += k.height + b[f]
        } else
            "button" == g.type && (g = g.button,
            h = g.createDom(),
            g.moveTo(c, e),
            g.show(),
            Blockly.bindEvent_(h, "mouseup", g, g.onMouseUp),
...
Blockly.VerticalFlyout.prototype.createCheckbox_ = function(a, b, c, d) {
    var e = a.getSvgRoot()
      , f = this.CHECKBOX_SIZE + this.CHECKBOX_MARGIN;
    b = Blockly.createSvgElement("rect", {
        "class": "blocklyFlyoutCheckbox",
        height: this.CHECKBOX_SIZE,
        width: this.CHECKBOX_SIZE,
        x: this.RTL ? this.getWidth() / this.workspace_.scale - f : b,
        y: c + d.height / 2 - this.CHECKBOX_SIZE / 2
    }, null);
    c = {
        svgRoot: b,
        clicked: !1,
    ...
    a.flyoutCheckbox = c;
    this.workspace_.getCanvas().insertBefore(b, e);
    this.checkboxes_.push(c)
}
;
Blockly.VerticalFlyout.prototype.checkboxClicked_ = function(a) {
    return function(b) {
        a.clicked = !a.clicked;
        a.clicked ? Blockly.addClass_(a.svgRoot, "checked") : Blockly.removeClass_(a.svgRoot, "checked");
        b.stopPropagation();
        b.preventDefault()
    }
}
...
Blockly.VerticalFlyout.prototype.setCheckboxState = function(a, b) {
    var c = this.workspace_.getBlockById(a);
    if (!c)
        throw "No block found in the flyout for id " + a;
    c = c.flyoutCheckbox;
    c.clicked = b;
    c.clicked ? Blockly.addClass_(c.svgRoot, "checked") : Blockly.removeClass_(c.svgRoot, "checked")
}
;
Blockly.VerticalFlyout.prototype.onMouseMove_ = function(a) {
    var b = this.getMetrics_();
    if (!(0 > b.contentHeight - b.viewHeight)) {
        var c = a.clientY - this.startDragMouseY_;
        this.startDragMouseY_ = a.clientY;
        a = b.viewTop - c;
        a = goog.math.clamp(a, 0, b.contentHeight - b.viewHeight);
        this.scrollbar_.set(a)
...
;
Blockly.VerticalFlyout.prototype.isDragTowardWorkspace_ = function(a, b) {
    var c = Math.atan2(b, a) / Math.PI * 180
      , d = !1
      , e = this.dragAngleRange_;
...
Blockly.VerticalFlyout.prototype.placeNewBlock_ = function(a) {
    ...
    var c = Blockly.getSvgXY_(c, b)
      ...
      , e = b.getMetrics();
    this.toolboxPosition_ == Blockly.TOOLBOX_AT_RIGHT && (scrollX = e.viewWidth - this.width_,
    c.x += scrollX / d - scrollX);
    this.parentToolbox_ && (c.y += this.parentToolbox_.getHeight() / b.scale - this.parentToolbox_.getHeight(),
    e = this.parentToolbox_.getWidth() / b.scale ,
    //- this.parentToolbox_.getWidth(), add by rongw 触屏
    c.x = this.toolboxPosition_ == Blockly.TOOLBOX_AT_RIGHT ? c.x + e : c.x - e);
...
;
if(!$('#blocklyDiv').hasClass('simple')){
Blockly.Toolbox.prototype.width = 141; //add by rongw
}else{
Blockly.Toolbox.prototype.width = 140; //add by rongw
}
...
Blockly.Toolbox.prototype.position = function() {
    ...
    if (a) {
        ...
        a.style.height = this.getHeight()+16 + "px",
...
Blockly.Toolbox.prototype.setSelectedItem = function(a) {
    ...
    this.selectedItem_ = a;
    null != this.selectedItem_ && (this.selectedItem_.setSelected(!0),
    this.flyout_.show(a.getContents()),//add by  rongw
...
Blockly.Toolbox.CategoryMenu.prototype.populate = function(a) {
    if (a) {
        for (var b = [], c = 0, d; d = a.childNodes[c]; c++)
            d.tagName && "CATEGORY" == d.tagName.toUpperCase() && b.push(d);
        a = Math.ceil(b.length / 1);
        for (c = 0; c < a; c += 1) {
            d = b[c];
            var e = goog.dom.createDom("tr", "scratchCategoryMenuRow");
            this.table.appendChild(e);
            this.categories_.push(new Blockly.Toolbox.Category(this,e,d));
            b[c + a] && this.categories_.push(new Blockly.Toolbox.Category(this,e,b[c + a]))
        }
        if($('body').hasClass('web-admin2')){
            this.height_ = $(window).height()-36;
        }else{
           this.height_ = $(window).height()-96; //侧边栏高度   add by rongw
        }
...
Blockly.Toolbox.CategoryMenu.prototype.dispose = function() {
    for (var a = 0, b; b = this.categories_[a]; a++)
        b.dispose();
    this.table && (goog.dom.removeNode(this.table),
    this.table = null)
}
;
Blockly.Toolbox.Category = function(a, b, c) {
    ...
    this.url_ = c.getAttribute("url");//add by rongw    添加image
    this.onclick_ = c.getAttribute("onclick");//add by rongw    添加image
    this.style_ = c.getAttribute("style");//add by rongw    添加image
    this.setColour(c);
...
Blockly.Toolbox.Category.prototype.dispose = function() {
    this.item_ && (goog.dom.removeNode(this.item_),
    this.item = null);
...
Blockly.Toolbox.Category.prototype.createDom = function() {
    ...
    var html=goog.dom.createDom("div", {
    });
    html.id="hasback";
    // html.style.height="60px";
    // html.style.lineHeight="60px";
    this.item_.appendChild(this.bubble_);
    if($('#hasback').length==0){
        this.parentHtml_.appendChild(html);
    }
...
Blockly.Toolbox.Category.prototype.setColour = function(a) {
    ...
    goog.isString(b) ? (b.match(/^#[0-9a-fA-F]{6}$/) ? this.colour_ = b : this.colour_ = Blockly.hueToRgb(b),
    a.match(/^#[0-9a-fA-F]{6}$/) ? this.secondaryColour_ = a : this.secondaryColour_ = Blockly.hueToRgb(a),
    this.hasColours_ = !0) : this.secondaryColour_ = this.colour_ = "#000000"
}
;
Blockly.Css = {};
Blockly.Css.Cursor = {
...
Blockly.Css.inject = function(a, b) {
    if (!Blockly.Css.styleSheet_) {
        ...
        var c = c.replace(/<<<PATH>>>/g, Blockly.Css.mediaPath_), d;
        for (d in Blockly.Colours)
            Blockly.Colours.hasOwnProperty(d) && (c = c.replace(new RegExp("\\$colour\\_" + d,"g"), Blockly.Colours[d]));
...
Blockly.Css.setCursor = function(a) {
    if (!(goog.userAgent.MOBILE || goog.userAgent.ANDROID || goog.userAgent.IPAD) && Blockly.Css.currentCursor_ != a) {
        Blockly.Css.currentCursor_ = a;
        var b;
        b = a == Blockly.Css.Cursor.OPEN ? "default" : "url(" + Blockly.Css.mediaPath_ + "/" + a + ".cur), auto";
        var c = ".blocklyDraggable {\n  cursor: " + b + ";\n}\n";
...
Blockly.WidgetDiv.hide = function(a) {
    ...
    Blockly.WidgetDiv.disposeAnimationTimer_ = null,
    Blockly.WidgetDiv.owner_ = null,
    Blockly.WidgetDiv.hideAndClearDom_()) : Blockly.WidgetDiv.isVisible() && (Blockly.WidgetDiv.dispose_ && Blockly.WidgetDiv.dispose_(),
    Blockly.WidgetDiv.dispose_ = null,
    Blockly.WidgetDiv.disposeAnimationFinished_ && !a ? Blockly.WidgetDiv.disposeAnimationTimer_ = window.setTimeout(Blockly.WidgetDiv.hide, 1E3 * Blockly.WidgetDiv.disposeAnimationTimerLength_) : (Blockly.WidgetDiv.disposeAnimationFinished_ && Blockly.WidgetDiv.disposeAnimationFinished_(),
    Blockly.WidgetDiv.disposeAnimationFinished_ = null,
    Blockly.WidgetDiv.owner_ = null,
...
;
Blockly.DragSurfaceSvg = function(a) {
    this.container_ = a
}
;
...
Blockly.DragSurfaceSvg.prototype.createDom = function() {
    if (!this.SVG_) {
        ...
        var a = Blockly.createSvgElement("defs", {}, this.SVG_);
        this.dragShadowFilterId_ = this.createDropShadowDom_(a);
        this.dragGroup_ = Blockly.createSvgElement("g", {}, this.SVG_);
        this.dragGroup_.setAttribute("filter", "url(#" + this.dragShadowFilterId_ + ")")
    }
}
;
Blockly.DragSurfaceSvg.prototype.createDropShadowDom_ = function(a) {
    a = Blockly.createSvgElement("filter", {
    ...
    Blockly.createSvgElement("feGaussianBlur", {
        "in": "SourceAlpha",
        stdDeviation: Blockly.DragSurfaceSvg.SHADOW_STD_DEVIATION
    }, a);
    var b = Blockly.createSvgElement("feComponentTransfer", {
        result: "offsetBlur"
    }, a);
...
;
Blockly.DragSurfaceSvg.prototype.setBlocksAndShow = function(a) {
    goog.asserts.assert(0 == this.dragGroup_.childNodes.length, "Already dragging a block.");
    this.dragGroup_.appendChild(a);
    this.SVG_.style.display = "block"
}
;
Blockly.DragSurfaceSvg.prototype.translateAndScaleGroup = function(a, b, c) {
    this.scale_ = c;
    a = a.toFixed(2);
    b = b.toFixed(2);
    Blockly.is3dSupported() ? this.dragGroup_.setAttribute("style", "transform: translate3d(" + a + "px, " + b + "px, 0px)scale3d(" + c + "," + c + "," + c + ")") : this.dragGroup_.setAttribute("transform", "translate(" + a + ", " + b + ") scale(" + c + ")")
}
;
Blockly.DragSurfaceSvg.prototype.translateSurface = function(a, b) {
    a *= this.scale_;
    b *= this.scale_;

    //add by rongw
    //flyoutset
    if(this.SVG_.classList=="blocklyDragSurface"){
    ...
    Blockly.is3dSupported() ? this.SVG_.setAttribute("style", "transform: translate3d(" + a+  "px, " + b + "px, 0px); display: block;") : this.SVG_.setAttribute("transform", "translate(" + a + ", " + b + ")")
}
;
Blockly.DragSurfaceSvg.prototype.getSurfaceTranslation = function() {
    var a = Blockly.getRelativeXY_(this.SVG_);
...
;
Blockly.DragSurfaceSvg.prototype.clearAndHide = function(a) {
...
Blockly.inject = function(a, b) {
    ...
    var e = Blockly.createDom_(d, c)
      , d = new Blockly.DragSurfaceSvg(d);
    d.createDom();
    c = Blockly.createMainWorkspace_(e, c, d);
    Blockly.init_(c);
    c.markFocused();
    Blockly.bindEvent_(e, "focus", c, c.markFocused);
    Blockly.svgResize(c);
    return c
}
;
Blockly.createDom_ = function(a, b) {
    ...
    b.stackGlowBlur = Blockly.createSvgElement("feGaussianBlur", {
        "in": "SourceGraphic",
        stdDeviation: Blockly.STACK_GLOW_RADIUS
    }, f);
    var g = Blockly.createSvgElement("feComponentTransfer", {
        result: "outBlur"
    }, f);
    Blockly.createSvgElement("feFuncA", {
        type: "table",
    ...
    g = Blockly.createSvgElement("feComponentTransfer", {
        result: "outBlur"
    }, f);
    Blockly.createSvgElement("feFuncA", {
        type: "table",
    ...
    g = Blockly.createSvgElement("pattern", {
...
;
Blockly.createMainWorkspace_ = function(a, b, c) {
    b.parentWorkspace = null;
    var d = new Blockly.WorkspaceSvg(b,c);
    d.scale = b.zoomOptions.startScale;
    a.appendChild(d.createDom("blocklyMainBackground"));
    d.translate(0, 0);
    d.markFocused();
    b.readOnly || b.hasScrollbars || d.addChangeListener(function() {
        if (Blockly.dragMode_ == Blockly.DRAG_NONE) {
            var a = d.getMetrics()
              , c = a.viewLeft + a.absoluteLeft
              , g = a.viewTop + a.absoluteTop;
            if (a.contentTop < g || a.contentTop + a.contentHeight > a.viewHeight + g || a.contentLeft < (b.RTL ? a.viewLeft : c) || a.contentLeft + a.contentWidth > (b.RTL ? a.viewWidth : a.viewWidth + c))
                for (var h = d.getTopBlocks(!1), k = 0, l; l = h[k]; k++) {
                    var m = l.getRelativeToSurfaceXY()
                      , n = l.getHeightWidth()
                      , p = g + 25 - n.height - m.y;
                    0 < p && l.moveBy(0, p);
                    p = g + a.viewHeight - 25 - m.y;
                    0 > p && l.moveBy(0, p);
                    p = 25 + c - m.x - (b.RTL ? 0 : n.width);
                    0 < p && l.moveBy(p, 0);
                    m = c + a.viewWidth - 25 - m.x + (b.RTL ? n.width : 0);
                    0 > m && l.moveBy(m, 0)
                }
        }
    });
...
Blockly.init_ = function(a) {
    ...
    b.horizontalLayout ? (a.scrollY = a.flyout_.height_,
    b.toolboxPosition == Blockly.TOOLBOX_AT_BOTTOM && (a.scrollY *= -1)) : (a.scrollX = a.flyout_.width_,
    b.toolboxPosition == Blockly.TOOLBOX_AT_RIGHT && (a.scrollX *= -1)),
    a.translate(a.scrollX, a.scrollY)));
...
Blockly.inject.loadSounds_ = function(a, b) {
    b.loadAudio_([a + "click.wav"], "click");
    b.loadAudio_([a + "delete.wav"], "delete");
    var c = []
      , d = function() {
        for (; c.length; )
    ...
    c.push(Blockly.bindEvent_(document, "mousemove", null, d, !0));
    c.push(Blockly.bindEvent_(document, "touchstart", null, d, !0))
}
;
...
Blockly.svgResize = function(a) {
    ...
    if (c) {
        ...
        var d = c.offsetWidth;
          ...
          //, c = c.offsetHeight;
        if(!$('#blocklyDiv').hasClass('simple')){
            var c = c.offsetHeight;
        }
        else{
            var c = c.offsetHeight;//-60; 是否预留msg的高度
        } //add by rongw
        b.cachedWidth_ != d && (b.setAttribute("width", d + "px"),
        b.cachedWidth_ = d);
        b.cachedHeight_ != c && (b.setAttribute("height", c + "px"),
 ...
 */
 ...
 ;
 Blockly.duplicatenext_ = function(a) {
     var b = Blockly.clipboardXml_
       , c = Blockly.clipboardSource_;
     Blockly.copynext_(a);
     a.workspace.paste(Blockly.clipboardXml_);
